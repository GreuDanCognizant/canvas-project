...
export function drawGrid(coords) {
    coords.ctx.save();
    ...
    for (let i = 10; i < coords.cx; i += 10) {
        coords.ctx.beginPath();
        coords.ctx.moveTo(i, 0);
        coords.ctx.lineTo(i, coords.cy);
        coords.ctx.stroke();
    }
    for (let j = 10; j < coords.cy; j += 10) {
        coords.ctx.beginPath();
        coords.ctx.moveTo(0, j);
        coords.ctx.lineTo(coords.cx, j);
        coords.ctx.stroke();
    }
    ...
    for (let i = 50; i < coords.cx; i += 10) {
        if (i % 50 === 0) {
            coords.ctx.beginPath();
            coords.ctx.moveTo(i, 0);
            coords.ctx.lineTo(i, 30);
            coords.ctx.stroke();
            coords.ctx.fillText(String(i), i, 30);
        }
        else {
            coords.ctx.beginPath();
            coords.ctx.moveTo(i, 0);
            coords.ctx.lineTo(i, 10);
            coords.ctx.stroke();
        }
    }
    for (let i = 50; i < coords.cy; i += 10) {
        if (i % 50 === 0) {
            coords.ctx.beginPath();
            coords.ctx.moveTo(0, i);
            coords.ctx.lineTo(30, i);
            coords.ctx.stroke();
            coords.ctx.fillText(String(i), 30, i);
        }
        else {
            coords.ctx.beginPath();
            coords.ctx.moveTo(0, i);
            coords.ctx.lineTo(10, i);
            coords.ctx.stroke();
        }
    }
    coords.ctx.restore();
